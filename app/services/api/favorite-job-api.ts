...
export class FavoriteJobAPI {
  ...
  async find(filter: Types.ShipperJobRequest | {} = {}): Promise<any> {
    try {
      ...
      const response: ApiResponse<any> = await this.apisauce.get('/api/v1/jobs/favorite', filter)
  ...
  async create(params: FavoriteJob): Promise<any> {
    try {
      ...
      const response: ApiResponse<any> = await this.apisauce.post(`/api/v1/jobs/favorite`, params)
